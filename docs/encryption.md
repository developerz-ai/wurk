...
| Job status (`track: true`) | Yes, without the result | The lifecycle, timings, progress and error are recorded as usual, but `perform`'s return value is **never stored** — a job handed an encrypted argument tends to return something derived from it, and the status row is a plaintext HASH. The row carries `result_withheld` instead, so a reader can tell the drop was policy rather than a `nil` return. This holds on the declaration alone: `encrypt: true` withholds the result even if `enable` was never called. |
