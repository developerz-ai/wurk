...
| `collapse:` (`{ policy: :debounce, wait:, max_wait: }` / `{ policy: :throttle, slot: }`) | ➕ Wurk extra | collapse repeat enqueues of one identity: **debounce** keeps the *last* payload and fires after `wait` seconds of quiet (capped at `max_wait` from the first enqueue), **throttle** admits one per epoch-aligned `slot` and drops the rest. Both return `nil` from `perform_async`. Mutually exclusive with `unique_for:`, rejected at class definition; not available on `perform_bulk` |
