...
5. ~~Dashboard: flow graph with per-node state, reusing the existing SPA patterns (lazy-loaded route + `components/Skeleton.tsx` fallback, per `CLAUDE.md`). Surface `abandoned` and a node's `broken` reason alongside the other states.~~ Done — `Wurk::Flow::Status` and `Wurk::FlowSet` are the canonical readers, and both surfaces go through them: the dashboard's `GET /api/flows`, `GET /api/flows/:fid` and `POST /api/flows/:fid/abandon` (step 6's kill switch, wired here), and the machine API's `GET /v1/flows/:fid`. `pages/Flows.tsx` lists, `pages/FlowDetail.tsx` draws the DAG — columns from a Kahn walk, since a forward-referenced `depends_on:` makes declaration order non-topological — over a node table carrying each `broken` link's reason. The listing walks headers only; node records are read lazily, in one pipeline. The /v1 side is read-only and has no listing: a producer holds the fid of the flow it created, and browsing or killing flows is an operator action with the graph on screen.
