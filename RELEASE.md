...
| 7 | **tag & GitHub Release** | `gh release create <tag> --target <sha>` creates the tag *and* the Release in one call, with `bin/release-notes` composing the notes — the CHANGELOG section plus generated conventional-commit notes — and the `.gem` attached (marked pre-release automatically for `Gem::Version#prerelease?`). **Last, deliberately** — see [the one rule](#the-one-rule-the-tag-is-an-output-not-an-input). |
